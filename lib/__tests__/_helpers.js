...
/**
 * Insert a Floorplan with a single table object — minimal seed for sceneBooking tests.
 */
export async function makeFloorplan({ restaurantId, tables = [{ objectId: 't1' }], ...rest } = {}) {
    return Floorplan.create({
        ...
        data: {
            objects: tables.map((t) => ({
                type: 'furniture',
                objectId: t.objectId,
                userData: new Map([['isTable', true], ['friendlyId', t.objectId]]),
            })),
        },
        ...rest,
    });
