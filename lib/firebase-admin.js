...
export const verifyFirebaseAuth = async (req) => {
  try {
    ...
    if (token.startsWith('line.')) {
      ...
      const uid = user.firebaseUid || `line:${lineUserId}`;
      return { success: true, uid, firebaseUid: uid, email: user.email, lineUserId, user };
    }

    // Custom customer JWT (from /api/login)
    try {
      const jwt = require('jsonwebtoken');
      const jwtSecret = process.env.JWT_SECRET || process.env.NEXTAUTH_SECRET;
      if (jwtSecret) {
        const decoded = jwt.verify(token, jwtSecret);
        if (decoded.isCustomer && decoded.userId) {
          const uid = decoded.userId.toString();
          return { success: true, uid, firebaseUid: uid, email: decoded.email, isCustomer: true };
        }
      }
    } catch (_) {
      // Not a valid custom JWT — fall through to Firebase
    }
    ...
    return { success: true, uid: decodedToken.uid, firebaseUid: decodedToken.uid, email: decodedToken.email, decodedToken };
  } catch (error) {
